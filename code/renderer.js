    ...
    sortedArchs.forEach(arch => {
        ...
        cardsHtml += `
            <div class="download-item">
                <div class="item-title">${archInfo.name}</div>
                <div class="item-details">
                    <span>${displayName}</span>
                    <span>${sizeFormatted}</span>
                </div>
                <a href="${mirrorPrefix}${originalUrl}" class="download-btn" target="_blank" rel="noopener" data-original-url="${originalUrl}">下载</a>
            </div>
        `;
    });
    
    return cardsHtml || '<div class="no-assets">暂无可用下载文件</div>';
}
...
function renderMgCards(assets, mirrorPrefix) {
    ...
    assets.forEach(asset => {
        ...
        cardsHtml += `
            <div class="download-item">
                <div class="item-title">MobileGlues</div>
                <div class="item-details">
                    <span>${displayName}</span>
                    <span>${sizeFormatted}</span>
                </div>
...
function renderJreCards(assets, mirrorPrefix) {
    ...
    assets.forEach(asset => {
        ...
        cardsHtml += `
            <div class="download-item">
                <div class="item-title">${versionName}</div>
                <div class="item-details">
                    <span>${displayName}</span>
                    <span>${sizeFormatted}</span>
                </div>
...
/**
 * 生成 ZalithLauncher2 下载卡片 HTML（按架构分组）
 */
function renderZalithCards(assets, mirrorPrefix) {
    // 按架构分组去重
    const archMap = new Map();
    assets.forEach(asset => {
        const arch = getArchFromFilename(asset.name);
